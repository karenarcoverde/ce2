...
def montar_q(lista_componentes,q,index):
	...
	vt0 = sp.symbols("vt0",real=True)
	jt0 = sp.symbols("jt0",real=True)

	## capacitor - metodo dos trapezios
	if (lista_componentes[index][0] == 'C' or lista_componentes[index][0] == 'c'):
		if (lista_componentes[index + 1] == 0):
			(q[int(lista_componentes[index + 2]) - 1][0]) += ((-2*v(t0)*(lista_componentes[index + 3]))/(deltat)) -j(t0)
		
	
		if (lista_componentes[index + 2] == 0):
			(q[int(lista_componentes[index + 1]) - 1][0]) += ((2*v(t0)*(lista_componentes[index + 3]))/(deltat)) +j(t0)
		
		elif ((lista_componentes[index + 1] != 0) and (lista_componentes[index + 2] != 0)):
			(q[int(lista_componentes[index + 1]) - 1][0]) += ((2*v(t0)*(lista_componentes[index + 3]))/(deltat)) +j(t0)
			(q[int(lista_componentes[index + 2]) - 1][0]) += ((-2*v(t0)*(lista_componentes[index + 3]))/(deltat)) -j(t0)
	
	## indutor - metodo dos trapezios
	if (lista_componentes[index][0] == 'X' or lista_componentes[index][0] == 'x'):
		(q[q.shape[0]-1][0]) += v(t0) + (2*(lista_componentes[index + 3])*j(t0))/(deltat)
...
def montar_yn(lista_componentes, yn, index, corrente,w,gama):
	...
	if (lista_componentes[index + 1] == 0):#corrente entrando no nó
		...
		if (lista_componentes[index][0] == 'C' or lista_componentes[index][0] == 'c'): 
			if (lista_componentes[index+4][0] == 'u(t)'):
			#capacitor
				(yn[int(lista_componentes[index + 2]) - 1][int(lista_componentes[index + 2]) - 1]) += 2*((lista_componentes[index + 3])/(deltat))
	...
	if (lista_componentes[index + 2] == 0): #corrente saindo do nó
		...
		##V
		if (lista_componentes[index][0] == 'V' or lista_componentes[index][0] == 'v'):
			print((yn[yn.shape[0]-1][int(lista_componentes[index + 1]) - 1]))
		...
		if (lista_componentes[index][0] == 'C' or lista_componentes[index][0] == 'c'): 
			if (lista_componentes[index+4][0] == 'u(t)'):
			#capacitor
				(yn[int(lista_componentes[index + 1]) - 1][int(lista_componentes[index + 1]) - 1]) += 2*((lista_componentes[index + 3])/(deltat))
			else:	
		...
		if (lista_componentes[index][0] == 'X' or lista_componentes[index][0] == 'x'): 
			if (lista_componentes[index+4][0] == 'u(t)'):
				(yn[int(lista_componentes[index + 1]) - 1][yn.shape[1]-1]) += 1		
				(yn[yn.shape[0]-1][int(lista_componentes[index + 1]) - 1]) += -1
				(yn[yn.shape[0]-1][yn.shape[1]-1]) += 2*((lista_componentes[index + 3]))/(deltat)
		...
		if (lista_componentes[index][0] == 'C' or lista_componentes[index][0] == 'c'): 
			...
			#capacitor
				(yn[int(lista_componentes[index + 1]) - 1][int(lista_componentes[index + 1]) - 1]) += 2*((lista_componentes[index + 3])/(deltat))
				(yn[int(lista_componentes[index + 2]) - 1][int(lista_componentes[index + 2]) - 1]) += 2*((lista_componentes[index + 3])/(deltat))
				(yn[int(lista_componentes[index + 1]) - 1][int(lista_componentes[index + 2]) - 1]) += -2*((lista_componentes[index + 3])/(deltat))
				(yn[int(lista_componentes[index + 2]) - 1][int(lista_componentes[index + 1]) - 1]) += -2*((lista_componentes[index + 3])/(deltat))
...
def menu():
	...
	if (opcao_linear == "nao linear"):
			...
			if (componente != '0'):
				...
				if (indice == 3):
					while ( (indice) <= (len(componente_descricao)-1)):
						if (lista_componentes[index][0] == 'V' or lista_componentes[index][0] == 'v' or lista_componentes[index][0] == 'I' or lista_componentes[index][0] == 'i' and lista_componentes[index+4][0] == 'u(t)'):
							componente_descricao_inteiro = componente_descricao[indice]
						else:
		...
		yn = zeros(maior_valor_nó, maior_valor_nó)
		q = zeros(maior_valor_nó,1)
		gama = zeros(2, 2)
		matriz_L = zeros(2, 2)
		...
		while ((index) < (len(lista_componentes))):
			if (type(lista_componentes[index]) is float):
				...
				if (lista_componentes[index][0] == 'V' or lista_componentes[index][0] == 'v'): #fonte de tensão independente
					...
					b = zeros(dimensao_yn[0] + 1,dimensao_yn[1] + 1)
					b[:-1,:-1] = yn
					yn = b
					#adiciona linha em q
					dimensao_q = (q.shape)
					b = zeros(dimensao_yn[0] + 1,1)
					b[:-1,:] = q
					q = b
					yn = montar_yn(lista_componentes,yn, index,corrente,w,gama)
					q = montar_q(lista_componentes,q, index)
